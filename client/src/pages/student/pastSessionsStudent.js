...
function PastSessionsStudent() {
    ...
    useEffect(() => {
        if (curUser == null) {
            ...
            navigate("/pastSessionsTutor", {replace: true})
        }
    ...
    const getAllSessions = () => {
        ...
        Axios.get('http://localhost:3001/api/studentPastSessions', {
            params: {
    ...
    const handleChange = (event) => {
        if (event.target.value !== 0) {
            Axios.get('http://localhost:3001/api/studentSubjectPastSessions', {
                params: {
                    studentId: curUser.id,
